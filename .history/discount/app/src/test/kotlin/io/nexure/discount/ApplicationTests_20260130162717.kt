...
/**
 * Integration tests for the Product API with PostgreSQL backend
...
class ApplicationTests {
    ...
    companion object {
        ...
        val postgres: PostgreSQLContainer<*> = PostgreSQLContainer("postgres:15-alpine")
            .apply {
                withDatabaseName("discount_test_db")
    ...
    @BeforeTest
    fun setUp() {
        // Initialize database with test container
        DatabaseConfig.init(
    ...
    fun testApplyDiscount() = testApplication {
        ...
        client.put("/products/laptop-se/discount") {
            ...
            setBody(Json.encodeToString(ApplyDiscountRequest.serializer(), discountRequest))
        }.apply {
            assertEquals(HttpStatusCode.OK, status)
            val responseBody = bodyAsText()
            assertTrue(responseBody.contains("Discount applied successfully"))
    ...
    fun testDiscountIdempotency() = testApplication {
        ...
        val requestBody = Json.encodeToString(ApplyDiscountRequest.serializer(), discountRequest)
        ...
        assertTrue(firstResponse.bodyAsText().contains("Discount applied successfully"))
        ...
        assertTrue(secondResponse.bodyAsText().contains("Discount already applied"))
    }
    ...
    fun testConcurrentDiscountApplication() = runBlocking {
        testApplication {
            ...
            // Verify the discount was applied exactly once
            val productRepository = ProductRepository()
    ...
    fun testMultipleDiscountPriceCalculation() = testApplication {
        ...
        val firstDiscount = ApplyDiscountRequest("discount1", 10.0)
        client.put("/products/laptop-de/discount") {
            contentType(ContentType.Application.Json)
            setBody(Json.encodeToString(ApplyDiscountRequest.serializer(), firstDiscount))
        }
        
        // Apply second discount
        val secondDiscount = ApplyDiscountRequest("discount2", 5.0)
        client.put("/products/laptop-de/discount") {
            contentType(ContentType.Application.Json)
            setBody(Json.encodeToString(ApplyDiscountRequest.serializer(), secondDiscount))
        }
        ...
        // basePrice = 1200, discounts = 10% + 5%
        // totalDiscount = 1 - (1-0.10) * (1-0.05) = 1 - 0.90 * 0.95 = 1 - 0.855 = 14.5%
        // priceAfterDiscount = 1200 * (1 - 0.145) = 1200 * 0.855 = 1026
        // finalPrice = 1026 * (1 + 0.19) = 1026 * 1.19 = 1220.94
        
        // The exact calculation should be in the response
        assertTrue(responseBody.contains("finalPrice"))
    }
