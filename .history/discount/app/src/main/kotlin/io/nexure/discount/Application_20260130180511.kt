...
fun Application.module() {
    // Initialize database connection first - this is critical
    DatabaseConfig.init()
    environment.log.info("Database connection initialized")
    
    // Initialize repository after database connection
    val productRepository = ProductRepository()
    
    // Seed database if needed - but handle any errors gracefully
    try {
        val existingProducts = productRepository.findAll()
        if (existingProducts.isEmpty()) {
            DataSeedingService.seedDatabase(productRepository)
            environment.log.info("Database seeded with sample products")
        } else {
            environment.log.info("Database already contains ${existingProducts.size} products. Skipping seed data.")
        }
    } catch (e: Exception) {
        environment.log.error("Failed to check or seed database: ${e.message}")
        throw e // Re-throw to prevent app from starting with bad database state
    }
    ...
    routing(productRepository)
}

fun Application.routing(productRepository: ProductRepository) {
    routing {
        /**
