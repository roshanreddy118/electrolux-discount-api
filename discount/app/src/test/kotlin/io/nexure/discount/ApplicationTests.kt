...
class ApplicationTests {
    ...
    fun testGetProductsWithValidCountry() = testApplication {
        ...
        client.get("/products?country=Sweden").apply {
            ...
            // Verify all returned products are for Sweden and have correct structure
            response.forEach { productElement ->
                val product = productElement.jsonObject
                
                // All products should be for Sweden
                assertEquals("Sweden", product["country"]?.jsonPrimitive?.content)
                
                // All products should have required fields
                assertNotNull(product["id"]?.jsonPrimitive?.content, "Product should have id")
                assertNotNull(product["name"]?.jsonPrimitive?.content, "Product should have name")
                assertNotNull(product["basePrice"]?.jsonPrimitive?.double, "Product should have basePrice")
                assertNotNull(product["finalPrice"]?.jsonPrimitive?.double, "Product should have finalPrice")
                
                // Verify VAT calculation for Sweden (25%)
                // finalPrice = basePrice * (1 - totalDiscount) * (1 + vatRate)
                val basePrice = product["basePrice"]?.jsonPrimitive?.double ?: 0.0
                val finalPrice = product["finalPrice"]?.jsonPrimitive?.double ?: 0.0
                
                // Calculate expected final price accounting for any discounts
                val discounts = product["discounts"]?.jsonArray ?: Json.parseToJsonElement("[]").jsonArray
                var discountMultiplier = 1.0
                discounts.forEach { discountElement ->
                    val percent = discountElement.jsonObject["percent"]?.jsonPrimitive?.double ?: 0.0
                    discountMultiplier *= (1.0 - percent / 100.0)
                }
                val expectedFinalPrice = basePrice * discountMultiplier * 1.25 // Sweden VAT is 25%
